...
func TestInitializeAzureAuthentication_EnvironmentVariables(t *testing.T) {
	...
	// Clean up after test
	defer func() {
		os.Setenv(envAzureClientID, originalClientID)
		os.Setenv(envAzureClientSecret, originalClientSecret)
		os.Setenv(envAzureTenantID, originalTenantID)
	}()

	// Test with all environment variables set
	t.Run("with all env vars set", func(t *testing.T) {
		os.Setenv(envAzureClientID, "test-client-id")
		os.Setenv(envAzureClientSecret, "test-client-secret")
		os.Setenv(envAzureTenantID, "test-tenant-id")
